...
def train_cnn(net, net_name, train_images, train_labels, testloader, epochs, resume_epoch=0, save_freq=40, batch_size=128, lr_base=0.01, lr_decay_factor=0.1, lr_decay_epochs=[80, 150], weight_decay=1e-4, path_to_ckpt = None, fn_denorm_labels=None):
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        for batch_idx in range(len(train_labels)//batch_size):
            ...
            # loss = torch.mean(torch.abs(outputs-batch_train_labels))

            # ### for test only
            # loss = torch.tensor([0],dtype=torch.float32).cuda()
            # distance = torch.abs(outputs-batch_train_labels)
