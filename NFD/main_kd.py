...
if feat_t.view(feat_t.size(0), -1).size(1)==feat_s.view(feat_s.size(0), -1).size(1):
    ...
    net_adapter = adapter(args.net_t_name, args.net_s_name).cuda()

## reconstruction network
net_recons = reconstruct(args.net_t_name, 1280).cuda()


if not args.do_CV:
    filename_ckpt = os.path.join(output_directory, 'ckpt_S_{}_T_{}_a_{}_s_{}_epoch_{}_last.pth'.format(args.net_s_name, args.net_t_name, args.alpha, args.sigma, args.epochs))
else:
    filename_ckpt = os.path.join(output_directory, 'ckpt_S_{}_T_{}_a_{}_s_{}_epoch_{}_last_inCV.pth'.format(args.net_s_name, args.net_t_name, args.alpha, args.sigma, args.epochs))
print('\n' + filename_ckpt)
...
if not os.path.isfile(filename_ckpt):
    ...
    train_kd(net_t=net_t, net_s=net_s, net_adapter=net_adapter, net_recons=net_recons, net_t_name=args.net_t_name, net_s_name=args.net_s_name, train_images=train_images, train_labels=train_labels, testloader=testloader, epochs=args.epochs, resume_epoch=args.resume_epoch, save_freq=args.save_freq, batch_size=args.batch_size_train, lr_base=args.lr_base, lr_decay_factor=args.lr_decay_factor, lr_decay_epochs=lr_decay_epochs, weight_decay=args.weight_decay, alpha=args.alpha, sigma=args.sigma, path_to_ckpt = path_to_ckpt_in_train, fn_denorm_labels=fn_denorm_labels)
...
if not args.do_CV:
    test_results_logging_fullpath = output_directory + '/test_results_S_{}_T_{}_a_{}_s_{}_epoch_{}_MAE_{:.3f}.txt'.format(args.net_s_name, args.net_t_name, args.alpha, args.sigma, args.epochs, test_mae)
else:
    test_results_logging_fullpath = output_directory + '/test_results_S_{}_T_{}_a_{}_s_{}_epoch_{}_MAE_{:.3f}_inCV.txt'.format(args.net_s_name, args.net_t_name, args.alpha, args.sigma, args.epochs, test_mae)
...
with open(test_results_logging_fullpath, 'a') as test_results_logging_file:
    ...
    test_results_logging_file.write("\n S_{}_T_{}_a_{}_s_{}; num paras: {}; seed: {} \n".format(args.net_s_name, args.net_t_name, args.alpha, args.sigma, num_parameters, args.seed))
