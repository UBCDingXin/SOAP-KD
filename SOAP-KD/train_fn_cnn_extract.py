...
def train_cnn_extract(net, net_name, train_images, train_labels, testloader, epochs, resume_epoch=0, save_freq=40, batch_size=128, lr_base=0.01, lr_decay_factor=0.1, lr_decay_epochs=[80, 150], weight_decay=1e-4, path_to_ckpt = None):
    ...
    assert train_labels.min()>=0 and train_labels.max()<23
    assert len(list(set(train_labels)))==23
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        test_acc = test_cnn_extract(net, testloader, verbose=False)
        test_acc_all.append(test_acc)
        
        print("{}: [epoch {}/{}] train_loss:{:.4f}, test_acc:{:.4f} Time:{:.4f}".format(net_name, epoch+1, epochs, train_loss, test_acc, timeit.default_timer()-start_time))
...
def test_cnn_extract(net, testloader, verbose=False):
